...
export function AIChatPanel({ isOpen, onClose, initialVoiceMode = false }: AIChatPanelProps) {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 429) throw new Error('System under heavy load. Please wait a moment.')
            if (response.status >= 500) throw new Error('AI service temporarily unavailable. We are investigating.')
            
            const errorData = await response.json().catch(() => ({}))
            throw new Error(errorData.error || 'Technical connectivity issue')
        }
        ...
        toast.error(err.message || "Connectivity issue. Please try again.")
        
        // Optionally add a system error message to the chat
        setMessages(prev => [...prev, {
            id: `error_${Date.now()}`,
            role: 'assistant',
            content: `⚠️ ERROR: ${err.message || "I'm having trouble connecting to my central processing core. Please check your connection."}`,
            timestamp: Date.now()
        }])
    } finally {
        setIsLoading(false)
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="bg-brand-dark px-6 py-6 flex items-center justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-brand-red/10 blur-3xl rounded-full" />
            <div className="flex items-center gap-3 relative z-10">
              <div className="w-10 h-10 bg-brand-red rounded-xl flex items-center justify-center transform -rotate-6 shadow-lg">
            ...
            <button onClick={onClose} className="p-2.5 bg-white/5 hover:bg-brand-red rounded-full transition-all text-white/40 hover:text-white relative z-10">
              <X className="w-4 h-4" />
            </button>
          </div>

          {/* Messages */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-brand-bg/30 subtle-scrollbar">
            {messages.map((msg) => (
              <div key={msg.id} className="space-y-3">
                ...
                >
                    ...
                    >
                    {msg.role === 'assistant' && <div className="absolute top-0 left-0 w-1 h-full bg-brand-red" />}
                    ...
                    >
                        {msg.widget.type === 'confirmation' ? (
                            <div className="flex gap-2">
                                <Button 
                                    className="flex-1 rounded-xl bg-brand-dark text-white text-[10px] uppercase font-black tracking-widest h-10"
                            ...
                            <div className="relative flex items-center">
                                <input 
                                    ...
                                    className="w-full bg-white border-gray-200 rounded-xl px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-brand-red transition-all"
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') handleSend((e.target as HTMLInputElement).value)
                                    }}
                                />
                                <button 
                                    className="absolute right-2 w-8 h-8 bg-brand-dark text-white rounded-lg flex items-center justify-center hover:bg-brand-red transition-colors"
                                    onClick={(e) => {
          ...
          <AnimatePresence>
            {isVoiceMode && (
              <motion.div
                ...
                className="bg-brand-dark flex flex-col items-center justify-center z-20 px-6 relative overflow-hidden"
              >
                ...
                <p className={cn(
                    ...
                    isSpeaking ? "text-brand-red" : "text-white"
                )}>
              ...
              <div className="flex items-center gap-1">
                <button
                    ...
                    isVoiceMode 
                        ...
                        : "text-gray-400 hover:bg-gray-200 hover:text-brand-dark"
                    )}
                >
                ...
                <Button
                    ...
                    className="rounded-lg bg-brand-dark hover:bg-brand-red w-9 h-9 p-0 transition-all duration-500 shadow-xl"
                >
