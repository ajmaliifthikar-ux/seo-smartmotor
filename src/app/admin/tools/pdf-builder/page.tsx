      ...
      <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-brand-red/10 flex items-center justify-center">
            <FileOutput className="w-4 h-4 text-brand-red" />
          </div>
          <div>
            <input value={meta.title} onChange={e => setMf('title', e.target.value)}
              className="font-black text-sm text-brand-dark bg-transparent outline-none border-b border-transparent focus:border-gray-300 transition-colors"
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setPreviewMode(!previewMode)}
            className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border',
              previewMode ? 'bg-brand-dark text-white' : 'bg-white border-gray-200 text-gray-600'
            )}>
          ...
          <button onClick={downloadPdf} disabled={generating}
            className={cn('flex items-center gap-1.5 px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-md',
              generating ? 'bg-gray-100 text-gray-400' : 'bg-brand-red text-white hover:bg-[#c91e23]'
            )}>
              ...
              {([
                ...
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={cn('flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-black uppercase tracking-wider transition-all border-b-2',
                    tab === t.id ? 'text-brand-red border-brand-red' : 'text-gray-400 border-transparent'
                  )}>
                  ...
                  <div className="space-y-1">
                    {TEMPLATES.map(tpl => (
                      <button key={tpl.id} onClick={() => loadTemplate(tpl)}
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-brand-red/[0.06] transition-all group text-left">
                        <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0 group-hover:bg-brand-red/10">
                          <tpl.icon className="w-3.5 h-3.5 text-gray-400 group-hover:text-brand-red" />
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-brand-dark">{tpl.name}</p>
                  ...
                  <div className="space-y-0.5">
                    {BLOCK_PALETTE.map(b => (
                      <button key={b.type} onClick={() => addBlock(b.type, b.defaultContent)}
                        ...
                        <b.icon className="w-3.5 h-3.5 text-gray-400 group-hover:text-brand-red flex-shrink-0" />
                  ...
                  <div className="grid grid-cols-4 gap-1.5 px-1">
                    {ACCENT_COLORS.map(c => (
                      <button key={c.value} onClick={() => setMf('accentColor', c.value)}
                        className={cn('w-full aspect-square rounded-xl border-2 transition-all', meta.accentColor === c.value ? 'border-brand-dark' : 'border-transparent')}
                  ...
                  {(['gradient', 'solid', 'minimal', 'none'] as const).map(s => (
                    <button key={s} onClick={() => setMf('headerStyle', s)}
                      className={cn('w-full px-3 py-2 rounded-xl text-xs font-bold transition-all border text-left capitalize',
                        meta.headerStyle === s ? 'bg-brand-dark text-white border-transparent' : 'bg-gray-50 border-gray-200 text-gray-600'
                      )}>{s}</button>
                  ))}
                </div>
              </div>
                ...
                {([
                  ...
                  <div key={f.key} className="space-y-1">
                    ...
                    <input value={meta[f.key]} onChange={e => setMf(f.key, e.target.value)}
                      className="w-full px-3 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-1 focus:ring-brand-red/30" />
                  </div>
                ))}
              </div>
            )}
                  ...
                  {blocks.map((block, idx) => (
                    <div key={block.id} onClick={() => setActiveBlockId(block.id)}
                      className={cn('group relative bg-white rounded-2xl border transition-all cursor-pointer p-4',
                        activeBlockId === block.id ? 'border-brand-red/40 shadow-md' : 'border-gray-100 hover:border-gray-200'
                      )}>
                      ...
                      <textarea value={block.content} onChange={e => updateContent(block.id, e.target.value)}
                        ...
                        className={cn('w-full bg-transparent outline-none resize-none placeholder-gray-300',
                          block.type === 'heading1' ? 'text-2xl font-black text-brand-dark' :
                          block.type === 'heading2' ? 'text-lg font-black text-brand-dark' :
