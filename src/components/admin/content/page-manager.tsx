            ...
            <div className="lg:col-span-3 space-y-6">
                <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-xl overflow-hidden">
                    <div className="flex items-center justify-between mb-10">
                        <div>
                            <h2 className="text-xl font-black text-brand-dark tracking-tighter uppercase">Modular Page Layout</h2>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Active Surface: {activePage}</p>
                        </div>
                        <Button className="bg-brand-red text-white rounded-2xl h-12 px-6 font-black uppercase tracking-widest text-[10px]">
                    ...
                    <Reorder.Group axis="y" values={pages[activePage] || []} onReorder={handleReorder} className="space-y-4">
                        {(pages[activePage] || []).map((block) => (
                            <Reorder.Item
                                key={block.key}
                                value={block}
                                className="group p-6 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between hover:border-brand-dark transition-colors cursor-grab active:cursor-grabbing"
                            >
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-brand-red mb-1">{block.type}</p>
                                        <h4 className="font-black text-brand-dark tracking-tighter uppercase">{block.key.split('_').join(' ')}</h4>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button className="p-3 rounded-xl bg-white text-gray-400 hover:text-brand-dark hover:shadow-md transition-all">
                                        <Edit3 size={18} />
                                    </button>
                                </div>
                            </Reorder.Item>
