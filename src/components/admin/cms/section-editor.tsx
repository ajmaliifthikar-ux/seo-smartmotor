  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute -inset-2 border-2 border-dashed border-brand-red/30 rounded-[2.5rem] pointer-events-none z-10"
          />
        )}
      </AnimatePresence>
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-0 group-hover/section:opacity-100 transition-all duration-300 transform translate-y-2 group-hover/section:translate-y-0">
        <div className="bg-brand-dark/90 backdrop-blur-xl border border-white/10 p-1.5 rounded-full shadow-2xl flex items-center gap-1">
          ...
          <button
            onClick={onEdit}
            className="p-2 rounded-full hover:bg-white/10 text-white hover:text-brand-red transition-all active:scale-90 flex items-center gap-2"
            title="Edit Content"
          >
            <Edit3 size={14} />
