  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button 
          type="button"
          className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 hover:border-brand-red hover:bg-brand-red/5 transition-all group"
        >
          {selectedIcon ? (
            <img 
            ...
            <>
              <div className="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-brand-red/10 group-hover:text-brand-red">
                <Search size={12} />
              </div>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] flex flex-col p-0 overflow-hidden bg-white rounded-[2rem] border-none shadow-2xl">
        <DialogHeader className="p-8 pb-4">
          <DialogTitle className="text-2xl font-black uppercase tracking-tighter italic">
            Icon <span className="text-brand-red">Library</span>
          </DialogTitle>
          <div className="relative mt-4">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
            <input
              placeholder="Search icons..."
              className="w-full bg-gray-50 border-none rounded-2xl py-3 pl-10 pr-4 text-xs font-bold uppercase tracking-widest focus:ring-2 focus:ring-brand-red/20 transition-all outline-none"
              value={search}
          ...
          {filteredIcons.map((icon) => (
            <button
              ...
              className={cn(
                ...
                selectedIcon === icon ? "border-brand-red bg-brand-red/5" : "border-gray-50 bg-gray-50/30 hover:border-gray-200"
              )}
            >
              <img 
