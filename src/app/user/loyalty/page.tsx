  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <FadeSection>
        ...
        <p className="text-sm text-brand-dark/50 mt-1">Earn points on every visit and redeem exclusive rewards</p>
      </FadeSection>

      {/* Points Hero */}
      <FadeSection delay={0.05}>
        <div className="carbon-fiber rounded-[2rem] relative overflow-hidden p-8">
          <div className="absolute inset-0 bg-gradient-to-br from-brand-dark/92 to-[#1a1a1a]/95" />
      ...
      <FadeSection delay={0.1}>
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-black/5">
          <h2 className="text-lg font-black uppercase tracking-tight text-brand-dark mb-5">
            Tier Progress
          </h2>
          ...
          {nextTier ? (
            <div className="flex items-center justify-between bg-brand-bg rounded-2xl p-4">
              <div>
                <p className="text-sm font-semibold text-brand-dark">
                  {(nextTier.min - USER_POINTS).toLocaleString()} pts to{' '}
                  <span style={{ color: nextTier.color }}>{nextTier.name}</span>
                </p>
                <p className="text-xs text-brand-dark/50 mt-0.5">Keep earning to unlock exclusive perks</p>
              </div>
              <ChevronRight className="w-5 h-5 text-brand-dark/30" />
            </div>
          ) : (
            <div className="text-center py-2">
              <p className="text-sm font-bold text-brand-dark">Maximum tier reached — Platinum status!</p>
              <p className="text-xs text-brand-dark/50 mt-1">Enjoy all exclusive Smart Motor benefits</p>
            </div>
          )}
        </div>
      </FadeSection>
      ...
      <FadeSection delay={0.1}>
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-black/5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-1.5 text-sm text-brand-dark/50">
              <Star className="w-4 h-4 text-[#FFD700]" />
              <span className="font-bold text-brand-dark">{USER_POINTS.toLocaleString()}</span> pts available
            </div>
          </div>
              ...
              return (
                <motion.div
                  ...
                  className={`border rounded-[1.5rem] p-5 transition-all duration-200 ${
                    canRedeem
                      ? 'border-black/8 hover:border-brand-red/30 hover:shadow-md cursor-pointer'
                ...
                >
                  ...
                  <h3 className="font-black text-brand-dark text-sm">{reward.name}</h3>
                  <p className="text-xs text-brand-dark/50 mt-1 mb-3">{reward.description}</p>
                  <button
                    disabled={!canRedeem}
                    className={`w-full py-2 rounded-xl text-xs font-bold transition-colors ${
                      canRedeem
                        ? 'bg-brand-red text-white hover:bg-[#cc1f25]'
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-brand-dark/8">
                  <th className="text-left text-xs font-bold text-brand-dark/50 uppercase tracking-wider pb-3">Date</th>
                  <th className="text-left text-xs font-bold text-brand-dark/50 uppercase tracking-wider pb-3">Action</th>
                  <th className="text-right text-xs font-bold text-brand-dark/50 uppercase tracking-wider pb-3">Points</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-brand-dark/5">
                {POINTS_HISTORY.map((item, i) => (
                  ...
                  >
                    <td className="py-3 text-xs text-brand-dark/50 font-medium whitespace-nowrap pr-4">{item.date}</td>
                    <td className="py-3 text-sm text-brand-dark font-semibold pr-4">{item.action}</td>
                    <td className="py-3 text-right">
                      <span
                        className={`text-sm font-black ${
                          item.type === 'earn' ? 'text-emerald-600' : 'text-brand-red'
                        }`}
                      >
      ...
      <FadeSection delay={0.1}>
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-black/5">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {HOW_TO_EARN.map((item, i) => (
              ...
              >
                ...
                <div>
                  <p className="font-black text-brand-dark text-sm">{item.label}</p>
                  <p className="text-2xl font-black mt-1" style={{ color: item.color }}>
                    +{item.points}
                    <span className="text-sm font-bold text-brand-dark/50 ml-1">pts</span>
                  </p>
                </div>
              </motion.div>
            ))}
          ...
          <div className="mt-5 p-4 bg-brand-red/5 border border-brand-red/15 rounded-2xl flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Gift className="w-5 h-5 text-brand-red flex-shrink-0" />
              <p className="text-sm font-semibold text-brand-dark">
            ...
            <Link
              href="/#booking"
              className="flex items-center gap-1 text-sm font-bold text-brand-red hover:underline flex-shrink-0"
            >
              Book Now
