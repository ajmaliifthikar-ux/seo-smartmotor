...
export function StatCard({ label, value, trend, trendUp, delay = 0, isLoading }: StatCardProps) {
  return (
    <DashboardWidget isLoading={isLoading} delay={delay} className="p-0">
      <div className="p-2">
        <p className="text-[10px] font-black uppercase tracking-widest text-gray-600 mb-2">{label}</p>
        <div className="flex items-end justify-between gap-2">
          <motion.p 
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            className="text-4xl font-black tracking-tighter text-brand-dark italic leading-none"
          >
            {value}
          </motion.p>
          <div className={cn(
