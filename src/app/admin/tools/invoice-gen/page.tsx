  ...
  return (
    <div className="relative">
      ...
      <input
        {...props}
        className={cn(
          'w-full py-2.5 rounded-xl border border-gray-200 text-sm text-brand-dark placeholder:text-gray-400 focus:ring-2 focus:ring-brand-red/30 focus:border-brand-red outline-none transition-all bg-white',
          Icon ? 'pl-8 pr-3' : 'px-3',
          props.className,
        )}
      />
...
function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-brand-dark placeholder:text-gray-400 focus:ring-2 focus:ring-brand-red/30 focus:border-brand-red outline-none transition-all bg-white resize-none"
    />
  )
}
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto print:bg-white print:p-0 print:block print:overflow-visible">
      ...
      <div className="print-modal-overlay w-full flex flex-col items-center">
        ...
        <div className="print-modal-controls sticky top-4 z-10 flex items-center gap-3 mb-6 bg-white/90 backdrop-blur rounded-2xl px-4 py-3 shadow-lg border border-gray-100 print:hidden">
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-brand-dark hover:bg-brand-red text-white rounded-xl px-5 py-2.5 text-xs font-black uppercase tracking-widest transition-all"
          >
        ...
        <div className="print-modal-card bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-gray-100 overflow-hidden print:shadow-none print:rounded-none print:border-0">
          ...
          {/* Header stripe */}
          <div className="bg-brand-dark px-10 py-8 flex items-start justify-between gap-6">
            {/* Brand */}
            <div>
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-brand-red font-black text-2xl tracking-tighter italic uppercase">Smart</span>
          ...
          <div className="px-10 py-6 grid grid-cols-2 gap-8 border-b border-gray-100 bg-gray-50/60">
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm font-bold text-brand-dark">{formatDate(invoice.date)}</p>
              </div>
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Due Date</p>
                <p className="text-sm font-bold text-brand-dark">{formatDate(invoice.dueDate)}</p>
              </div>
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">Bill To</p>
              <p className="font-black text-brand-dark text-base">{invoice.customerName || '—'}</p>
              ...
              {(invoice.vehicleMake || invoice.vehiclePlate) && (
                <div className="mt-2 inline-flex items-center gap-1.5 bg-brand-red/8 text-brand-red text-[11px] font-bold px-2.5 py-1 rounded-lg">
                  <Car size={11} />
                  ...
                  return (
                    <tr key={item.id} className={cn('border-b border-gray-50', idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')}>
                      <td className="py-3 pr-4">
                        <p className="font-semibold text-brand-dark leading-snug">{item.description || <span className="text-gray-300 italic">—</span>}</p>
                      </td>
                      ...
                      <td className="py-3 text-right font-bold text-brand-dark">AED {formatAED(net)}</td>
                    </tr>
                  )
                })}
              </tbody>
            ...
            <div className="mt-6 flex justify-end">
              <div className="w-72 space-y-2">
                ...
                <div className="flex justify-between items-center pt-3 border-t-2 border-brand-dark">
                  <span className="text-base font-black text-brand-dark uppercase tracking-wide">Total</span>
                  <span className="text-xl font-black text-brand-dark">AED {formatAED(totals.total)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="bg-brand-dark/5 border-t border-gray-100 px-10 py-5 flex items-center justify-between">
    ...
    <>
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-brand-dark italic leading-none">
              Invoice Generator
            </h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 border border-gray-200 text-gray-500 hover:border-gray-400 hover:text-brand-dark rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all"
            >
              <RotateCcw size={12} /> Reset
            </button>
            <button
              onClick={() => setShowPreview(true)}
              className="flex items-center gap-2 bg-brand-red hover:bg-[#c41e24] text-white rounded-xl px-5 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-brand-red/20"
            >
            ...
            <SectionCard title="Line Items" icon={ClipboardList}>
              ...
              <div className="mb-5">
                <button
                  ...
                  className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-brand-red hover:text-[#c41e24] transition-colors"
                >
                  <Tag size={12} />
                  ...
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                    {SERVICE_PRESETS.map(preset => (
                      <button
                        ...
                        className="group relative text-left bg-gray-50 hover:bg-brand-red/5 border border-gray-200 hover:border-brand-red/30 rounded-xl px-3 py-2.5 transition-all"
                      >
                        <p className="text-[10px] font-black text-brand-dark group-hover:text-brand-red transition-colors uppercase tracking-wide leading-tight">{preset.label}</p>
                ...
                {invoice.items.map((item, idx) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-brand-dark placeholder:text-gray-300 focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none bg-white transition-all"
                    />
                    {/* Qty */}
                    <input
                      type="number"
                      ...
                      className="w-full px-2 py-2 rounded-lg border border-gray-200 text-sm text-center text-brand-dark focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none bg-white transition-all"
                    />
                    {/* Unit Price */}
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-2 py-2 rounded-lg border border-gray-200 text-sm text-right text-brand-dark focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none bg-white transition-all"
                      />
                    </div>
                    {/* Discount */}
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-2 pr-6 py-2 rounded-lg border border-gray-200 text-sm text-right text-brand-dark focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none bg-white transition-all"
                      />
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-9 h-9 rounded-lg border border-transparent text-gray-300 hover:text-brand-red hover:border-brand-red/20 hover:bg-brand-red/5 transition-all disabled:opacity-20 disabled:cursor-not-allowed"
              ...
              <button
                onClick={addItem}
                className="mt-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-brand-dark transition-colors px-1"
              >
          ...
          <div className="xl:sticky xl:top-8 space-y-4">
            ...
            <div className="bg-brand-dark rounded-2xl p-6 text-white">
              ...
              <div className="space-y-3">
                ...
                {totals.totalDiscount > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-brand-red/80">Discount</span>
                    <span className="text-sm font-bold text-brand-red">− AED {formatAED(totals.totalDiscount)}</span>
                  </div>
                )}
                <div className="flex justify-between items-center">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
              ...
              <button
                onClick={() => setShowPreview(true)}
                className="w-full flex items-center justify-center gap-2 bg-brand-red hover:bg-[#c41e24] text-white rounded-xl py-3 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-brand-red/20"
              >
                <Eye size={13} /> Preview Invoice
              </button>
              ...
              <button
                ...
                className={cn(
                  ...
                  copiedTotal
                    ? 'bg-emerald-50 border-emerald-200 text-emerald-600'
                    : 'bg-gray-50 border-gray-200 text-gray-600 hover:border-gray-400 hover:text-brand-dark',
                )}
              >
              ...
              <button
                onClick={addItem}
                className="w-full flex items-center justify-center gap-2 border border-gray-200 text-gray-500 hover:border-brand-dark hover:text-brand-dark rounded-xl py-3 text-[10px] font-black uppercase tracking-widest transition-all"
              >
                <Plus size={13} /> Add Line Item
              </button>
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex items-baseline gap-1.5 mb-1">
                <span className="text-brand-red font-black text-lg tracking-tighter italic">Smart</span>
                <span className="text-brand-dark font-black text-lg tracking-tighter italic">Motor</span>
              </div>
