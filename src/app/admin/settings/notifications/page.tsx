    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-2 bg-brand-red text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-[#c8181e] disabled:opacity-50 transition-colors"
        >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-base font-bold text-gray-900 mb-1 flex items-center gap-2">
          <Mail className="w-4 h-4 text-brand-red" /> Alert Recipients
        </h2>
        ...
        <div className="flex gap-2">
          <input type="email" value={newEmail} onChange={e => setNewEmail(e.target.value)}
            ...
            className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red" />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-base font-bold text-gray-900 mb-1 flex items-center gap-2">
          <Megaphone className="w-4 h-4 text-brand-red" /> Event Alerts
        </h2>
          ...
          {EVENT_TYPES.map(event => {
            ...
            return (
              <div key={event.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
                enabled ? 'border-brand-red/20 bg-brand-red/5' : 'border-gray-100 bg-gray-50'
              }`}>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => toggleEvent(event.id)}
                    className={`relative inline-flex w-10 h-5 rounded-full transition-colors ${enabled ? 'bg-brand-red' : 'bg-gray-300'}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-base font-bold text-gray-900 flex items-center gap-2">
            <Clock className="w-4 h-4 text-brand-red" /> Daily Report
          </h2>
          <button onClick={() => setSettings(s => ({ ...s, dailyReportEnabled: !s.dailyReportEnabled }))}
            className={`relative inline-flex w-10 h-5 rounded-full transition-colors ${settings.dailyReportEnabled ? 'bg-brand-red' : 'bg-gray-300'}`}>
          ...
          <div className="space-y-5">
            <div>
              ...
              <input type="time" value={settings.dailyReportTime}
                ...
                className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red" />
            </div>
            <div>
      ...
      <div className="bg-gray-900 rounded-2xl p-6 text-white">
        <h2 className="text-sm font-bold mb-4 flex items-center gap-2">
          <Info className="w-4 h-4 text-brand-red" /> Setup Checklist
        </h2>
        <ol className="space-y-3 text-sm text-gray-300">
          {[
            ...
            <li key={i} className="flex items-start gap-3">
              <span className="shrink-0 w-5 h-5 bg-brand-red text-white text-xs font-bold rounded-full flex items-center justify-center mt-0.5">{i + 1}</span>
              <span>{step}</span>
            </li>
          ))}
        </ol>
