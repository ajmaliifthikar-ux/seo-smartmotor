        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-brand-dark tracking-tighter uppercase">Activity Logs</h2>
                    ...
                    {history.audit.map((log) => (
                        <div key={log.id} className="p-6 rounded-[1.5rem] border border-gray-100 bg-white shadow-sm hover:shadow-md transition-all">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-[10px] font-black uppercase tracking-widest text-brand-red bg-brand-red/5 px-3 py-1 rounded-full">
                                    {log.entityType}
                                </span>
                                <span className="text-[10px] font-bold text-gray-400">
                                    {new Date(log.timestamp?.toDate?.() || log.timestamp || log.createdAt?.toDate?.() || log.createdAt).toLocaleString()}
                                </span>
                            </div>
                            <h4 className="text-sm font-black text-brand-dark uppercase tracking-tighter mb-2">{log.key}</h4>
            ...
            <div className="space-y-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-brand-dark tracking-tighter uppercase">State Snapshots</h2>
                        ...
                        <div key={snap.id} className="p-6 rounded-[2rem] border border-gray-100 bg-white shadow-sm overflow-hidden group">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xs font-bold text-brand-dark">{new Date(snap.createdAt?.toDate?.() || snap.createdAt).toLocaleString()}</span>
                                </div>
                                <Button
                                    variant="ghost"
                                    onClick={() => handleRestore(snap.id)}
                                    className="h-8 rounded-full text-[10px] font-black uppercase tracking-widest text-brand-red hover:bg-brand-red hover:text-white"
                                >
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-brand-dark uppercase tracking-tighter">{snap.key}</p>
                                </div>
                                <button 
