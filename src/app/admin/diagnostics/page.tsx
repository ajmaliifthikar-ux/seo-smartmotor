...
export default async function DiagnosticsPage() {
    ...
    return (
        <div className="space-y-10">
            {/* Header */}
            <div>
                <h1 className="text-3xl font-black tracking-tighter text-brand-dark uppercase italic">
                ...
                {realtimeHealth.map((health: any) => (
                    <Card key={health.service} className="p-8 rounded-[2.5rem] bg-white border border-gray-100 shadow-sm relative overflow-hidden group">
                        <div className={cn(
                            ...
                            health.status === 'WORKING' ? "bg-green-500" : "bg-brand-red"
                        )} />
                        ...
                        <div className="space-y-1">
                            <p className="text-2xl font-black text-brand-dark">{health.status}</p>
                                ...
                                <tr key={trace.id} className="group hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-8 py-6">
                                        <span className={cn(
                                            ...
                                            trace.duration > 1000 ? "text-brand-red" : "text-gray-500"
                                        )}>
                                            {trace.duration}ms
                                        </span>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-2">
                                            <div className={cn(
                                                ...
                                                trace.status === 'SUCCESS' ? "bg-green-500" : "bg-brand-red"
                                            )} />
                                            <span className={cn(
                                                "text-[10px] font-black uppercase tracking-widest",
                                                trace.status === 'SUCCESS' ? "text-green-600" : "text-brand-red"
                                            )}>
                                                {trace.status}
                                            </span>
                                        </div>
