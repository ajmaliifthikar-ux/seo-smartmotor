        ...
        <div className="flex flex-wrap gap-2">
          {CONVERSIONS.map(c => (
            ...
            >
              ...
              <ArrowRight size={10} />
              <span className="font-mono text-[10px] bg-brand-red/10 px-1.5 py-0.5 rounded uppercase text-brand-red">{c.to}</span>
            </button>
          ))}
        </div>
              ...
              {files.map(cf => (
                <div key={cf.id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-brand-dark truncate">{cf.name}</p>
                  ...
                  <div className="flex items-center gap-2">
                    {cf.status === 'converting' && <RefreshCw size={15} className="animate-spin text-brand-red" />}
                    {cf.status === 'done' && cf.resultBlob && (
                      <button onClick={() => download(cf)} className="flex items-center gap-1 text-xs font-bold text-brand-red hover:underline whitespace-nowrap">
