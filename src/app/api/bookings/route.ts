    ...
    try {
        ...
        const bookingData = await traceIntegration(
            ...
            async () => {
                ...
                if (bookingResult && bookingResult.bookingRef) {
                    // Send Email Notification
                    try {
                        await sendBookingConfirmation(data.email, {
                            id: bookingResult.bookingRef,
                            serviceName: data.serviceId, // Better to resolve actual name, but we have ID
                            date: data.date,
                            time: data.time,
        ...
        return NextResponse.json({ success: true, bookingId: bookingData?.id, bookingRef: bookingData?.bookingRef })
