    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-brand-dark italic">QR Code Generator</h1>
                    <p className="text-sm text-gray-500 mt-1">Generate trackable QR codes for campaigns and marketing</p>
                </div>
                <button
                    onClick={() => setShowForm(v => !v)}
                    className="flex items-center gap-2 bg-brand-red hover:bg-[#c41e24] text-white rounded-xl px-5 py-3 text-xs font-black uppercase tracking-widest transition-all shadow-lg"
                >
                    <Plus size={14} />
                    New QR Code
                </button>
                    ...
                    <form onSubmit={handleGenerate} className="space-y-5">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-9 pr-4 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-red focus:border-transparent outline-none"
                                />
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-red outline-none bg-white"
                                >
                        ...
                        <div className="flex gap-3 pt-2">
                            <button
                                ...
                                className="flex items-center gap-2 bg-brand-dark hover:bg-brand-red text-white rounded-xl px-6 py-3 text-xs font-black uppercase tracking-widest transition-all disabled:opacity-50"
                            >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                    ...
                    <button onClick={loadQRCodes} className="text-gray-400 hover:text-brand-red transition-colors">
                        <RefreshCw size={16} />
                    </button>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                        {qrCodes.map(item => (
                            <div key={item.id} className="group border border-gray-100 rounded-2xl p-5 hover:border-brand-red/20 hover:shadow-lg transition-all duration-300">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="text-[9px] font-black bg-brand-red/10 text-brand-red px-2 py-0.5 rounded-full uppercase tracking-wider">
                                                {item.scans || 0} scans
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-50">
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-gray-500 hover:text-brand-dark transition-colors"
                                        title="Download PNG"
                                    >
                                    ...
                                    <button
                                        ...
                                        className={cn('flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider transition-colors', copied === item.id ? 'text-green-600' : 'text-gray-500 hover:text-brand-dark')}
                                        title="Copy URL"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-gray-500 hover:text-brand-red transition-colors"
                                    >
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 relative" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="bg-brand-red/5 rounded-2xl p-4 text-center">
                                <p className="text-3xl font-black text-brand-red">{selected.scans || 0}</p>
                                        ...
                                        {sorted.map(([key, count]) => (
                                            <div key={key}>
                                                ...
                                                <div className="w-full bg-gray-100 rounded-full h-1.5">
                                                    <div className="bg-brand-red h-1.5 rounded-full" style={{ width: `${total > 0 ? ((count as number) / total) * 100 : 0}%` }} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <button
                            ...
                            className="w-full mt-4 flex items-center justify-center gap-2 bg-brand-dark hover:bg-brand-red text-white rounded-xl py-3 text-xs font-black uppercase tracking-widest transition-all"
                        >
