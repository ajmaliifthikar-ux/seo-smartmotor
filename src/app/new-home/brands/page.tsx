                    ...
                    <section id={category.id} key={category.id} className="scroll-mt-32">
                        <div className="max-w-7xl mx-auto px-6 md:px-12">
                            <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-gray-100 pb-8">
                                <div>
                                    <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-4 text-brand-dark">{category.name}</h2>
                                    <p className="text-gray-500 max-w-xl">{category.description}</p>
                                </div>
                                <div className="hidden md:block text-brand-red">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {category.brands.map((brand) => {
                                    ...
                                    <Link href={`/new-home/brands/${brand.slug || brand.id}`} key={brand.id} className="group block h-full">
                                        <div className="bg-white rounded-[2rem] p-8 transition-all duration-500 h-full flex flex-col border border-gray-100 hover:border-brand-red/30 relative overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-brand-red/10 group-hover:-translate-y-2">
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-br from-brand-dark to-black opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                                                ...
                                                <div className="flex justify-between items-start mb-8">
                                                    <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center shadow-lg border border-gray-50 group-hover:scale-110 transition-transform duration-500 p-2">
                                                        ...
                                                        {brand.logoUrl ? (
                                                            ...
                                                            <span className="text-brand-dark font-black text-xs uppercase text-center leading-none">{brand.name}</span>
                                                        )}
                                                    </div>
                                                    <div className="w-10 h-10 rounded-full bg-gray-100 group-hover:bg-brand-red flex items-center justify-center transition-colors duration-300">
                                                        <ArrowRight className="text-gray-400 group-hover:text-brand-red transition-colors" size={18} />
                                                    </div>
                                                </div>

                                                <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 text-brand-dark group-hover:text-brand-red transition-colors">{brand.name}</h3>
