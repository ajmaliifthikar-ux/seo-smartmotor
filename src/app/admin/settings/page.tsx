...
function Field({
  ...
  return (
    <div className="space-y-1.5">
      <label className="block text-xs font-bold text-brand-dark uppercase tracking-widest">
        {label}
      </label>
      {children}
...
function Input({
  ...
  return (
    <div className={cn('flex items-center bg-gray-50 border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-brand-red/20 focus-within:border-brand-red/50 transition-all', className)}>
      {prefix && (
      ...
      <input
        ...
        className="flex-1 px-4 py-2.5 text-sm text-brand-dark bg-transparent outline-none placeholder-gray-300"
      />
    </div>
  )
}
...
function Textarea({
  ...
  return (
    <textarea
      ...
      placeholder={placeholder}
      rows={rows}
      className="w-full px-4 py-2.5 text-sm text-brand-dark bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red/50 transition-all resize-none placeholder-gray-300"
    />
  )
}
...
function SaveToast({ show, error }: { show: boolean; error?: string }) {
  ...
  return (
    <div className={cn(
      ...
      error ? 'bg-red-500' : 'bg-brand-dark'
    )}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-brand-dark flex items-center gap-2">
        ...
        <button
          ...
          className={cn(
            ...
            saving
              ...
              : 'bg-brand-red text-white hover:bg-[#c91e23] shadow-red-200 hover:shadow-red-300'
          )}
        >
          {saving ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <Field label="Currency">
                <select
                  ...
                  className="w-full px-4 py-2.5 text-sm text-brand-dark bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red/50"
                >
              ...
              {DAYS.map(day => {
                ...
                return (
                  <div key={day} className="flex items-center gap-4 py-2.5 px-4 rounded-xl bg-gray-50 border border-gray-100">
                    <span className="w-24 text-sm font-bold text-brand-dark">{day}</span>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <div
                        ...
                        className={cn(
                          ...
                          h.open ? 'bg-brand-red' : 'bg-gray-200'
                        )}
                      ...
                      <span className={cn('text-xs font-semibold w-12', h.open ? 'text-brand-red' : 'text-gray-400')}>
                        {h.open ? 'Open' : 'Closed'}
                      </span>
                    </label>
                    {h.open && (
                      <div className="flex items-center gap-2 ml-auto">
                        <input
                          ...
                          className="text-sm text-brand-dark bg-white border border-gray-200 rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-brand-red/20"
                        />
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <button
                  key={q.label}
                  onClick={q.action}
                  className="px-4 py-2 rounded-xl text-xs font-bold bg-gray-100 text-gray-600 hover:bg-brand-red/10 hover:text-brand-red transition-all border border-gray-200 hover:border-brand-red/30"
                >
                  {q.label}
                </button>
              ))}
          ...
          <Section title="Pricing & Tax" description="Configure default rates applied to all services">
            ...
            <div className="flex flex-col gap-3 pt-2">
              <label className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50 border border-gray-100">
                <div>
                  <p className="text-sm font-bold text-brand-dark">Enable VAT on all services</p>
                  <p className="text-xs text-gray-400 mt-0.5">Apply {vatRate}% VAT automatically to all line items</p>
                </div>
                <div
                  onClick={() => setVatEnabled(!vatEnabled)}
                  className={cn('relative rounded-full cursor-pointer transition-all duration-200', vatEnabled ? 'bg-brand-red' : 'bg-gray-200')}
              ...
              {[
                ...
                <label key={row.label} className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50 border border-gray-100">
                  ...
                  <div
                    ...
                    className={cn('relative rounded-full cursor-pointer transition-all duration-200', row.val ? 'bg-brand-red' : 'bg-gray-200')}
                    style={{ width: 40, height: 22, flexShrink: 0 }}
                  >
              ...
              <Field label="Current Password">
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 pr-12 text-sm text-brand-dark bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red/50"
                  />
                    ...
                    {[8, 12, 16, 20].map(threshold => (
                      <div
                        ...
                        className={cn(
                          ...
                          newPw.length >= threshold ? 'bg-brand-red' : 'bg-gray-200'
                        )}
                      />
                    ))}
                  </div>
          ...
          <Section title="Two-Factor Authentication" description="Add an extra layer of security to your account">
            <div className="flex items-center justify-between p-4 rounded-xl bg-gray-50 border border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-brand-dark">Authenticator App</p>
                  <p className="text-xs text-gray-400">Google Authenticator / Authy</p>
                </div>
              </div>
              <div
                onClick={() => setTwoFactor(!twoFactor)}
                className={cn('relative rounded-full cursor-pointer transition-all duration-200', twoFactor ? 'bg-brand-red' : 'bg-gray-200')}
            ...
            <div className="space-y-5">
              <Field label="Session Timeout (minutes)" hint="Admin is logged out after this period of inactivity">
                <select
                  ...
                  className="w-full max-w-xs px-4 py-2.5 text-sm text-brand-dark bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20"
                >
              ...
              <label className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50 border border-gray-100">
                <div>
                  <p className="text-sm font-bold text-brand-dark">Login Notifications</p>
                  <p className="text-xs text-gray-400 mt-0.5">Receive email when a new session starts</p>
                </div>
                <div
                  onClick={() => setLoginNotif(!loginNotif)}
                  className={cn('relative rounded-full cursor-pointer transition-all duration-200', loginNotif ? 'bg-brand-red' : 'bg-gray-200')}
                  style={{ width: 40, height: 22, flexShrink: 0 }}
                >
