    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-brand-dark flex items-center gap-2">
          <Type className="w-6 h-6 text-brand-red" />
          Font Pairing
        </h1>
            ...
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setFilter(c)}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-bold transition-all border',
                  filter === c ? 'bg-brand-dark text-white border-transparent' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'
                )}>{c}</button>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {MOODS.map(m => (
              <button key={m} onClick={() => setMood(m)}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-bold transition-all border',
                  mood === m ? 'bg-brand-red text-white border-transparent' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'
                )}>{m}</button>
            ))}
          </div>
        </div>
          ...
          {filtered.map(p => (
            <div
              ...
              className={cn(
                ...
                selected?.id === p.id
                  ? 'border-brand-red/40 bg-brand-red/[0.02] shadow-md ring-1 ring-brand-red/20'
                  : 'border-gray-100 bg-white hover:border-gray-200'
              )}
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-sm font-black text-brand-dark">{p.name}</h3>
              ...
              <div className="flex items-center gap-4 mt-3">
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-black text-brand-red uppercase tracking-wider">H:</span>
          ...
          <div className="space-y-4 sticky top-4 self-start">
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-black text-brand-dark">{selected.name}</h3>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    copied ? 'bg-green-100 text-green-600' : 'bg-brand-red text-white hover:bg-[#c91e23]'
                  )}
                >
              ...
              <div className="px-5 py-4 border-t border-gray-50 bg-gray-50/30 grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-wider text-brand-red mb-1">Heading Font</p>
                  <p className="text-sm font-bold text-brand-dark">{selected.heading}</p>
                  ...
                  <a href={`https://fonts.google.com/specimen/${selected.heading.replace(/ /g, '+')}`}
                    ...
                    className="flex items-center gap-1 text-[10px] text-brand-red hover:underline mt-1 font-bold">
                    Google Fonts <ExternalLink size={9} />
                  </a>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase tracking-wider text-gray-400 mb-1">Body Font</p>
                  <p className="text-sm font-bold text-brand-dark">{selected.body}</p>
