  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-lg font-black uppercase tracking-tight text-brand-dark">{title}</h2>
      </div>
      {children}
    </motion.div>
  )
...
function Toggle({ checked, onChange, label, description }: {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-brand-dark/5 last:border-0">
      <div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`w-full px-4 py-3 rounded-2xl border text-sm font-bold transition-all
          ${disabled
            ? 'bg-gray-50 border-black/5 text-gray-400 cursor-not-allowed'
            : 'bg-white border-black/10 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-red/25 focus:border-brand-red'
          }`}
      />
      {disabled && (
        <p className="text-[10px] text-gray-400 mt-1 pl-1">This field cannot be changed</p>
      )}
    </div>
  )
}
...
export default function UserSettingsPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      >
        <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tighter italic text-brand-dark">
          Settings
        </h1>
        <p className="text-sm text-brand-dark/50 mt-1">Manage your account, vehicles and preferences</p>
      </motion.div>

      {/* Profile Section */}
      <SectionCard title="Profile" icon={User}>
        <div className="flex items-center gap-5 mb-6 pb-6 border-b border-brand-dark/8">
          <div className="relative">
            <div className="w-20 h-20 rounded-[1.5rem] bg-gradient-to-br from-brand-red/20 to-brand-dark/10 flex items-center justify-center">
              <User className="w-9 h-9 text-brand-red/60" />
            </div>
            <button className="absolute -bottom-1 -right-1 w-7 h-7 bg-brand-red rounded-xl flex items-center justify-center shadow-md hover:bg-[#cc1f25] transition-colors">
              <Camera className="w-3.5 h-3.5 text-white" />
            </button>
          </div>
          <div>
            <p className="font-black text-brand-dark text-lg">{profile.name}</p>
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-brand-red text-white text-sm font-bold rounded-full hover:bg-[#cc1f25] transition-colors disabled:opacity-60"
        >
      ...
      <SectionCard title="My Vehicles" icon={Car}>
        <div className="space-y-3 mb-4">
          {vehicles.map((veh) => (
            <motion.div
              ...
              className="flex items-center justify-between p-4 bg-brand-bg rounded-2xl border border-brand-dark/8"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-brand-dark/8 flex items-center justify-center">
                  <Car className="w-5 h-5 text-brand-dark/50" />
                </div>
                <div>
                  <p className="font-bold text-brand-dark text-sm">
                    {veh.brand} {veh.model}
                  </p>
                  <p className="text-xs text-brand-dark/50">{veh.year} &middot; {veh.plate}</p>
                </div>
              </div>
              <button
          ...
          {vehicles.length === 0 && (
            <div className="text-center py-6 text-sm text-brand-dark/40">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm font-bold text-brand-red hover:underline mb-4"
        >
        ...
        {showAddVehicle && (
          <motion.div
            ...
            className="border border-brand-red/20 rounded-2xl p-4 bg-brand-red/5"
          >
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-2xl border border-brand-dark/15 bg-white text-sm font-medium text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-red/25 focus:border-brand-red appearance-none"
                  >
      ...
      <SectionCard title="Security" icon={Shield}>
        <h3 className="text-sm font-bold text-brand-dark mb-4">Change Password</h3>
            ...
            <div key={field}>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-4 py-3 pr-11 rounded-2xl border border-brand-dark/15 bg-white text-sm font-medium text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-red/25 focus:border-brand-red"
                />
                <button
                  type="button"
                  onClick={() => setShowPw((p) => ({ ...p, [field]: !p[field] }))}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 text-brand-dark/40 hover:text-brand-dark/70 transition-colors"
                >
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-brand-dark/8 text-brand-dark text-sm font-bold rounded-full hover:bg-brand-dark/15 transition-colors"
          >
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder="DELETE"
            className="w-full px-4 py-3 rounded-2xl border border-red-200 bg-white text-sm font-bold text-brand-dark focus:outline-none focus:ring-2 focus:ring-red-300 focus:border-red-400 max-w-xs"
          />
        </div>
