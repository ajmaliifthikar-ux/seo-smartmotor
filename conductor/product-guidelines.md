...
## 6. Staff-Facing Agentic OS
- **Departmental Isolation:** Each administrative department (Strategy Lab, Workbench, etc.) should maintain a distinct visual accent and specialist AI context.
- **Proactive Assistance:** The system must actively monitor data and push intelligence to the staff (via Notification Center), rather than waiting for manual queries.
- **Multimodal Intelligence:** Administrative tools must support rich data inputs including document uploads (PDF/CSV/XLSX) and visual analysis.

## 7. Autonomous Agent Orchestration
- **Transparency:** Every autonomous action taken by an agent must be logged in the Audit Trail with a link to the reasoning (e.g., "Ranking drop detected").
- **Human-in-the-Loop:** High-impact optimizations (e.g., public content changes) must require a "One-Click Approval" from an administrator before execution.
- **Fail-Open Safety:** If a data source (e.g., GSC API) or memory layer (Redis) is unavailable, agents must gracefully degrade to local mock data or skip the task rather than crashing.
