...
export function RecentActivityLive() {
  ...
  useEffect(() => {
    if (!db) {
      console.error('RecentActivityLive: Firestore not initialized')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="space-y-4">
      {activities.map((activity) => (
        <div 
          key={activity.id}
          className="flex items-center justify-between p-4 rounded-2xl border border-gray-50 bg-white hover:border-brand-red/10 transition-all group"
        >
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-brand-dark group-hover:text-white transition-colors">
              <UserIcon size={18} />
            </div>
            <div>
          ...
          <div className="text-right">
            <div className="flex items-center justify-end gap-1.5 text-brand-red">
              <Clock size={12} />
