...
function BarRow({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-xs font-black text-brand-dark w-12 text-right">{fmtNum(value)}</span>
    </div>
  )
}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-brand-dark">Website Analytics</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={fetchData}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 bg-brand-dark text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-brand-red transition-colors disabled:opacity-50"
          >
      ...
      {error && (
        <div className="rounded-3xl border border-red-100 bg-red-50 p-8 text-center">
          ...
          <button onClick={fetchData} className="text-xs font-black text-brand-red uppercase tracking-widest hover:underline">
            Retry
          </button>
        </div>
      )}
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-base font-black text-brand-dark">Daily Traffic</h2>
                ...
                <div className="flex items-center gap-4 text-[10px] font-bold">
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 rounded bg-brand-dark inline-block" />
                    Sessions
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 rounded bg-brand-red inline-block" />
                    Users
                  </span>
                </div>
              </div>
                ...
                {data.daily.map((d, i) => {
                  ...
                  return (
                    <div key={d.date} className="flex-1 flex flex-col items-center gap-0.5 group relative">
                      {/* Tooltip */}
                      <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-brand-dark text-white text-[8px] px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                        {d.date.slice(5)}<br />{d.sessions}s / {d.users}u
                      </div>
                      <div className="w-full flex items-end gap-0.5 h-40">
                        <div
                          className="flex-1 bg-brand-dark rounded-t transition-all duration-500 group-hover:bg-brand-red"
                          style={{ height: `${sessionH}%` }}
                        />
                        <div
                          className="flex-1 bg-brand-red/40 rounded-t transition-all duration-500 group-hover:bg-brand-red/70"
                          style={{ height: `${userH}%` }}
                        />
                      </div>
                      {i % 5 === 0 && (
            ...
            <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
              <h2 className="text-base font-black text-brand-dark mb-1">Traffic Channels</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Devices */}
            <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
              <h2 className="text-base font-black text-brand-dark mb-1">Devices</h2>
                    ...
                    <div key={d.device} className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-brand-dark capitalize">{d.device}</span>
                        </div>
            ...
            {/* Countries */}
            <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
              <h2 className="text-base font-black text-brand-dark mb-1">Top Countries</h2>
            ...
            <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm flex flex-col">
              <h2 className="text-base font-black text-brand-dark mb-1">Trend Overview</h2>
              ...
              <div className="flex-1 space-y-6">
                <div>
                  ...
                  <div className="flex justify-between text-[9px] text-gray-400 font-medium mt-1">
                    <span>30d ago</span>
                    <span className="font-black text-brand-dark">{fmtNum(ov!.sessions)} total</span>
                    <span>today</span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex justify-between text-[9px] text-gray-400 font-medium mt-1">
                    <span>30d ago</span>
                    <span className="font-black text-brand-red">{fmtNum(ov!.activeUsers)} total</span>
                    <span>today</span>
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-3xl border border-gray-100 bg-white overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-gray-50 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-black text-brand-dark">Top Pages</h2>
                    ...
                    <tr key={p.path} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-brand-dark">{p.path}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right text-xs font-black text-brand-dark">{fmtNum(p.views)}</td>
