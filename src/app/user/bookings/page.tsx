  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          <h3 className="font-black text-brand-dark text-base leading-tight">{booking.serviceName}</h3>
          {booking.vehicle && (
            <p className="text-xs text-brand-dark/50 mt-0.5">{booking.vehicle}</p>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm text-brand-dark/60">
        ...
        {booking.cost && (
          <span className="ml-auto font-bold text-brand-dark">{booking.cost}</span>
        )}
      </div>
    </motion.div>
  )
  ...
  return (
    <motion.div
      ...
      className={`flex gap-4 p-4 rounded-[1.5rem] border transition-colors cursor-pointer
        ${notif.read ? 'bg-white border-black/5' : 'bg-brand-red/5 border-brand-red/20'}`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className={`text-sm font-bold ${notif.read ? 'text-brand-dark/70' : 'text-brand-dark'}`}>
            {notif.title}
          </p>
        ...
        <p className="text-xs text-brand-dark/50 mt-0.5 leading-relaxed">{notif.body}</p>
        <p className="text-xs text-brand-dark/40 mt-1.5 font-medium">{notif.timestamp}</p>
      </div>
    </motion.div>
  )
}
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-brand-dark/50 mt-1">Track your appointments and notifications</p>
        </div>
        <Link
          href="/#booking"
          className="flex items-center gap-2 px-5 py-2.5 bg-brand-red text-white text-sm font-bold rounded-full hover:bg-[#cc1f25] transition-colors shadow-md shadow-brand-red/20"
        >
          ...
          >
            {loading ? (
              ...
              <div className="bg-white rounded-[2rem] p-8 text-center border border-black/5">
                ...
                <p className="text-sm font-semibold text-brand-dark/60">{error}</p>
              </div>
            ) : bookings.length === 0 ? (
              <motion.div
              ...
              >
                <CalendarCheck className="w-16 h-16 text-brand-dark/15 mx-auto mb-4" />
                <h3 className="text-lg font-black text-brand-dark mb-2">No bookings yet</h3>
                <p className="text-sm text-brand-dark/50 mb-6">
                  Schedule your first service and keep your vehicle in top shape.
                </p>
                <Link
                  href="/#booking"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-brand-red text-white font-bold rounded-full hover:bg-[#cc1f25] transition-colors"
                >
                  Book a Service
