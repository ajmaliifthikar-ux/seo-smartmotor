...
export class StrategyAgent {
    ...
    private async getInternalMetrics() {
        ...
        const completedBookings = bookings.filter(b => b.status === 'completed' || (b.status as string) === 'COMPLETED').length
        const faqViews = analytics.filter((a: any) => a.eventType === 'FAQ_VIEW').length
