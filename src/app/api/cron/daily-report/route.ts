...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (bookingsSnap.status === 'fulfilled') {
      bookingsSnap.value.docs.forEach((doc: any) => {
        const d = doc.data()
        bookingsTotal++
    ...
    if (urlsSnap.status === 'fulfilled') {
      urlsSnap.value.docs.forEach((doc: any) => {
        const d = doc.data()
    ...
    const recipientsStr = process.env.NOTIFICATION_EMAILS || process.env.ADMIN_EMAIL || ''
    const recipients = recipientsStr.split(',').map(e => e.trim()).filter(Boolean)

    if (recipients.length === 0) {
      console.warn('No report recipients configured.')
