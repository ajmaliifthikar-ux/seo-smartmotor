...
function ColorSwatch({
  ...
  const textCls = isLight ? 'text-brand-dark' : 'text-white'
        ...
        {formats.map(({ key, label, value }) => (
          <div key={key} className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="flex-shrink-0 p-1.5 rounded-lg hover:bg-gray-100 transition-colors text-gray-400 hover:text-brand-red"
              title={`Copy ${label}`}
            >
              {copiedKey === key ? (
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200">
      ...
      <div className="bg-white p-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black text-brand-dark">{name}</p>
...
function TintChip({
  ...
  const [r, g, b] = hexToRgb(hex)
  const isLight = luminance(r, g, b) > 0.35
  const textCls = isLight ? 'text-brand-dark/70' : 'text-white/80'
  ...
  return (
    <button
      ...
      className={`relative flex flex-col items-center justify-end h-20 rounded-xl transition-all duration-150 hover:scale-105 hover:shadow-lg ${
        base ? 'ring-2 ring-offset-2 ring-brand-dark' : ''
      }`}
      style={{ backgroundColor: hex }}
    >
      ...
      {copied && (
        <span className="absolute inset-0 flex items-center justify-center">
          <Check size={14} className={isLight ? 'text-brand-dark' : 'text-white'} />
        </span>
      )}
    </button>
  )
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tighter text-brand-dark italic">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="w-28 px-3 py-2 rounded-xl border border-gray-200 text-sm font-mono focus:ring-2 focus:ring-brand-red focus:border-transparent outline-none"
                    maxLength={7}
                  />
                </div>
              </div>
              ...
              <div className="flex-1 w-full sm:w-auto">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-red focus:border-transparent outline-none"
                />
              </div>

              <button
                onClick={handleAddColor}
                className="flex items-center gap-2 bg-brand-red hover:bg-[#c41e24] text-white rounded-xl px-5 py-3 text-xs font-black uppercase tracking-widest transition-all shadow-lg whitespace-nowrap"
              >
                <Plus size={14} />
                Add Color
              </button>
              ...
              {customColors.length > 0 && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-gray-200 text-xs font-black uppercase tracking-wider text-gray-600 hover:border-brand-dark transition-all"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                >
                  ...
                  <p className="text-sm font-black text-brand-dark mt-1">{item.ratio}</p>
                    ...
                    <button
                      ...
                      className={`w-8 h-8 rounded-lg border-2 transition-all hover:scale-110 ${
                        tintBase.toLowerCase() === hex.toLowerCase()
                          ? 'border-brand-dark shadow-lg scale-110'
                          : 'border-gray-200'
                      }`}
                      style={{ backgroundColor: hex }}
                    />
                      ...
                      >
                        ...
                        <td className="py-1.5 pr-6">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span
                              className="font-mono cursor-pointer hover:text-brand-red transition-colors"
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-brand-dark hover:bg-brand-red text-white rounded-xl px-5 py-3 text-xs font-black uppercase tracking-widest transition-all shadow-lg"
              >
            ...
            <pre className="bg-brand-dark text-brand-bg rounded-2xl p-6 text-xs font-mono leading-relaxed overflow-x-auto">
              {cssBlock.split('\n').map((line, i) => {
                if (line === ':root {' || line === '}') {
                  return (
                ...
                if (match) {
                  return (
                    <span key={i} className="block">
                      <span>{match[1]}</span>
                      <span className="text-brand-red">{match[2]}</span>
                      <span className="text-gray-400">{match[3]}</span>
                      <span className="text-brand-bg">{match[4]}</span>
...
.hero {
  ...
  color: var(--color-light-bg);
                ...
                <div key={title}>
                  ...
                  <pre className="bg-brand-dark text-brand-bg rounded-xl p-4 text-[11px] font-mono overflow-x-auto leading-relaxed">
                    {code}
                  </pre>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <button
                  key={label}
                  onClick={action}
                  className="flex items-center gap-2 px-5 py-3 rounded-xl border border-gray-200 text-xs font-black uppercase tracking-wider text-gray-700 hover:bg-brand-dark hover:text-white hover:border-brand-dark transition-all"
                >
                  <Icon size={13} />
                  {label}
                </button>
      ...
      <div className="bg-brand-dark rounded-2xl px-6 py-4 flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
