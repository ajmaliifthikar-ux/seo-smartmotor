  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="space-y-1.5 flex-1 min-w-[120px]">
            ...
            <input type="number" value={globalW} onChange={e => setGlobalW(e.target.value)}
              className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20" />
          </div>
          <button
            onClick={() => setLockAspect(!lockAspect)}
            title={lockAspect ? 'Aspect ratio locked' : 'Aspect ratio unlocked'}
            className={cn('p-2 rounded-xl border transition-all self-end mb-0', lockAspect ? 'bg-brand-red/10 border-brand-red/30 text-brand-red' : 'bg-gray-50 border-gray-200 text-gray-400')}
          >
          ...
          <div className="space-y-1.5 flex-1 min-w-[120px]">
            ...
            <input type="number" value={globalH} onChange={e => setGlobalH(e.target.value)}
              disabled={lockAspect}
              className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-red/20 disabled:opacity-40" />
          </div>
      ...
      <div
        ...
        className="border-2 border-dashed border-gray-200 rounded-2xl p-10 flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-brand-red/40 hover:bg-brand-red/[0.02] transition-all"
      >
              ...
              {images.map(img => (
                <div key={img.id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-brand-dark truncate">{img.name}</p>
                  ...
                  <div className="w-20 flex items-center justify-center">
                    {img.status === 'processing' && <RefreshCw size={16} className="animate-spin text-brand-red" />}
                    {img.status === 'done' && img.resultBlob && (
                      <button onClick={() => dlOne(img)} className="flex items-center gap-1 text-xs font-bold text-brand-red hover:underline">
          ...
          <div className="flex gap-3">
            <button
              ...
              className={cn(
                ...
                processing ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-brand-red text-white hover:bg-[#c91e23] shadow-red-200'
              )}
            >
              {processing
            ...
            {done && (
              <button onClick={downloadAll}
                className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black uppercase tracking-wider bg-brand-dark text-white hover:bg-gray-800 transition-all shadow-md">
