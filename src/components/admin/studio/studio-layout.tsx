      ...
      <div className="w-[350px] flex flex-col gap-4 bg-white/50 backdrop-blur-xl rounded-[2.5rem] border border-gray-100 p-6 overflow-hidden shadow-sm">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xl font-black uppercase tracking-tighter italic">
            {title} <span className="text-brand-red text-xs not-italic ml-2">({items.length})</span>
          </h2>
          <Button 
            onClick={onAddItem}
            className="w-8 h-8 rounded-full p-0 bg-brand-dark text-white hover:bg-brand-red transition-all"
          >
            <Plus size={16} />
          </Button>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-gray-50 border-none rounded-2xl py-2.5 pl-9 pr-4 text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-brand-red/20 transition-all outline-none"
            value={searchQuery}
          ...
          {filteredItems.map((item) => (
            ...
            >
              {item.image ? (
                ...
                <div className="w-10 h-10 rounded-xl bg-brand-red/10 flex items-center justify-center flex-shrink-0 text-brand-red">
