    ...
    return (
        <div className="space-y-12 pb-20">
            ...
            <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.03)]">
                <div className="flex flex-col md:flex-row gap-6 items-end">
                    <div className="flex-1 space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-brand-red px-2">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full h-16 bg-gray-50 border-none rounded-2xl px-6 text-sm font-medium focus:ring-2 focus:ring-brand-dark transition-all"
                            />
                    ...
                    <button
                        ...
                        className="h-16 px-10 bg-brand-dark text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-brand-red disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-3 shadow-xl hover:shadow-brand-red/20"
                    >
                        {isAnalyzing ? (
                            <>
            ...
            {report && (
                ...
                >
                    {/* Main Score Card */}
                    <div className="lg:col-span-1 bg-brand-dark rounded-[2.5rem] p-10 text-white flex flex-col items-center justify-center text-center">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {report.technicalLogs.issues.map((issue: string, i: number) => (
                                    <div key={i} className="flex items-start gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                        <AlertCircle size={18} className="text-brand-red mt-1 shrink-0" />
                                        <p className="text-sm font-bold text-brand-dark leading-tight">{issue}</p>
                                    </div>
                                ))}
                            ...
                            <div className="space-y-4">
                                {report.recommendations.map((rec, i) => (
                                    <motion.div
                                        ...
                                        className="flex items-center gap-4 p-5 bg-gradient-to-r from-gray-50 to-white rounded-2xl border border-gray-100 hover:border-brand-dark transition-colors"
                                    >
                                        <div className="w-8 h-8 rounded-full bg-brand-dark text-white flex items-center justify-center text-xs font-black shrink-0">
                                            {i + 1}
                                        </div>
                                        <p className="text-sm font-medium text-brand-dark">{rec}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </div>
